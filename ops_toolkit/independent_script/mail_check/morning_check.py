        ...
        if img_dir is not None:
            ...
            for f_name in filelist:
                ...
                if extension in [".jpg", ".jpeg", ".png", ".bmp"]:
                    Path(img_dir).joinpath(zipfile.Path(f_name).name).write_bytes(
                        zipf.read(f_name)
                    )
...
class MorningCheck:
    ...
    def report(self):
        ...
        self.report_path.joinpath(f"morning_check_{self.today}.html").write_text(
            SCRIPT_DIR.joinpath("./template_report_morning_check.html")
            .read_text(encoding="utf-8")
            .replace(
                "__ROW_REPORTS_JSON__",
                self.report_path.joinpath(f"morning_check_{self.today}.json").read_text(
                    encoding="utf-8"
                ),
            )
            .replace("__REPORT_DATE__", self.today),
            encoding="utf-8",
        )
    ...
    def report_to_teams(self):
        ...
        send_teams_message(url, title, errs + lab)

    def get_mail(self, subject, time_start, time_end, /, step: Step, msg="", **kwargs):
        folder = os.getenv("MORNING_CHECK_FOLDER", "InBox")
        try:
            mail = self.mail_manager.search_email_newest(
                subject, time_start, time_end, folder=folder, **kwargs
            )
        except Exception as e:
            logger.error(e)
            mail = None
        ...
        step.add_report(
            ...
            bool(mail),
            (
                f"{mail.subject} \n发送时间：{mail.get_send_time().strftime('%Y-%m-%d %H:%M:%S')}"
                if mail
                else "未找到邮件"
            ),
        ...
        if mail:
            logger.info(
                f"✅找到邮件：{mail.subject} 发送时间：{mail.get_send_time().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            return mail
        else:
            return None
    ...
    def _main(self, *args):
        for func in args:
            ...
            try:
                ...
                _s.add_report(
                    "Error Msg", False, str(e), "", "程序错误，检查完整日志排查"
                )

            finally:
                if isinstance(_s, Step):
                    _s.to_reports(
                        self.report_path.joinpath(f"morning_check_{self.today}.json")
                    )
    ...
    def main_0901(self):
        self._main(self.check_2_2_0901_daily_pushlog_order_status)
    ...
    def check_1_06_esunny(self) -> Step:
        ...
        counts_ok = self.findstr(
            "正常|忽略", self.docx_to_text(mail.get_attachment())
        ).__len__()
        counts_err = self.findstr(
            "异常|提醒", self.docx_to_text(mail.get_attachment())
        ).__len__()
        _s.add_report(
            "报告中正常或忽略出现的次数 ",
    ...
    def check_1_09_fcca(self) -> Step:
        ...
        for sub in [
            ...
            "NS (NSBC) Batch Job Status -  SUCCESSFUL",
            "NS (NSSC) Batch Job Status -  SUCCESSFUL",
            "NS (NSOR) Batch Job Status -  SUCCESSFUL",
            "NS (NSDS-PRE) Batch Job Status -  SUCCESSFUL",
            "NS (NSDS) Batch Job Status -  SUCCESSFUL",
            "NS (NSWL-PRE) Batch Job Status -  SUCCESSFUL",  # T
    ...
    def check_1_10_0630_daily_push_log_hkg_status_no(self) -> Step:
        ...
        for mk, cg, stat in self.findstr(
            r"<td>(HKG)</td><td>(\w*?)\W*?</td><td>(\w{2})</td></tr>",
            mail.get_html(),
            _s,
            "HKG Market Status 完整性验证",
            4,
        ):
            logger.debug(f"HKG and CSC Market Status 匹配到行： {mk} {cg} {stat}")
            # if cg in ('ETS', 'GEM', 'MAIN', 'NASD'):
            _s.add_report(
                f"HKG and CSC Market Status (Market = {cg})", stat == "NO", stat, "NO"
            )

        # ===== Have record for DataFeed Record except IndexData =====
        for mk, record in self.findstr(
            r"<tr><td>(HKG|SHA|SZA|188.*?|81.*?)</td><td>(\d+)</td></tr>",
            mail.get_html(),
            _s,
            "No of DataFeed Record: 数据匹配完整性",
            15,
        ):
            logger.debug(f"No of DataFeed Record 匹配到行： {mk=} {record=}")
            if mk in ("188 IndexData", "81 IndexData"):
                _s.add_report(
                    f"No of DataFeed Record: ({mk})", record == "0", record, "0"
                )
            else:
                _s.add_report(
                    f"No of DataFeed Record: ({mk})", int(record) > 0, record, "> 0"
                )
    ...
    def check_1_11_tp1_bix_gateway_status(self) -> Step:
        _s = Step(
            1,
            11,
            "08:15 TTL Securities FO BIX Check (Email: TP1 PROD-Health Check Notification-PROD)",
        )
        logger.info(_s)
        ...
        _gs = self.findstr(
            ...
            mail.get_html(),
            _s,
            "BIX Gateway Status: 数据匹配完整性",
            9,
        )
        for bid, ip, port, conn, logon in _gs:
            logger.debug(
                f"No of DataFeed Record 匹配到行： {bid, ip, port, conn, logon}"
            )
            _s.add_report(
    ...
    def check_1_12_tp1_mds_status(self) -> Step:
        _s = Step(
            1,
            12,
            "08:15 TTL Secutiries FO MDS Check (Email: TP1 PROD-Health Check Notification-PROD)",
        )
        logger.info(_s)
        mail = self.get_mail(
            "TP1 PROD-Health Check Notification-PROD",
        ...
        if _mds:
            _s.add_report(
                ...
                bool(
                    _mds[0][3] == "TRUE"
                    and _mds[0][4].replace(" ", "") == "MAMK,USA,SZMK,BOD,HKEX"
                ),
                f"{_mds}",
    ...
    def check_1_13_tp1_external_bix_status(self) -> Step:
        ...
        for bid in ["BIX-FCFIX2", "BIX-FIXIDM", "BIX-FCFIX"]:
            _g = self.findstr(
                ...
                mail.get_html(),
                _s,
                f"TP1 External BIX Gateway - {bid} 数据匹配完整性",
            )
            if _g:
                _s.add_report(
    ...
    def check_1_14_tp1_fo_market_info(self) -> Step:
        _s = Step(
            1,
            14,
            "08:15 TTL Securities FO Market Info Check (Email: TP1 PROD-Health Check Notification-PROD)",
        )
        logger.info(_s)
        mail = self.get_mail(
            "TP1 PROD-Health Check Notification-PROD",
        ...
        def _is_ok(_act: list[str], _exp: str):
            ...
            if "LastTradeDate" in _exp:
                _rst.append(
                    _act[1] == self.trade_date.last_date_hk.strftime("%Y-%m-%d")
                )
        ...
        for mk, exp in market_info:
            _g = self.findstr(
                r"<tr><td>(%s)</td><td>(.{10})</td><td>(.{10})</td><td>(TRUE|FALSE)</td><td>(TRUE|FALSE)</td><td>(TRUE|FALSE)</td><td>(TRUE|FALSE)</td></tr>"
                % mk,
                mail.get_html(),
                _s,
                f"{mk}: 数据匹配",
    ...
    def check_1_15_tp1_mq(self) -> Step:
        _s = Step(
            1,
            15,
            "08:15 TTL Securities FO MQ Connection Check (Email: TP1 PROD-Health Check Notification-PROD)",
        )
        logger.info(_s)
        mail = self.get_mail(
            "TP1 PROD-Health Check Notification-PROD",
    ...
    def check_1_18_daily_pushlog(self) -> Step:
        ...
        mail = self.get_mail(
            "Daily PushLog Checking",
            self.trade_date.current.strftime("%Y-%m-%d 08:17"),
            self.trade_date.current.strftime("%Y-%m-%d 08:20"),
            _s,
        )
        if mail is None:
            return _s
        ...
        ocg = self.findstr(
            r"<td>(OCG Dealer)</td><td>(\d{8})</td><td>(.{16}).*?</td>",
            mail.get_html(),
            _s,
            "BSS Logon Date 正则匹配失败",
        )
        if ocg:
            _hk_current = self.trade_date.current.strftime("%Y%m%d")
            _s.add_report(
            ...
            _send_time = self.trade_date.current.strftime("%Y-%m-%dT08:18")
            _s.add_report(
        ...
        _ldc = self.findstr(
            r"<tr><td>Last Dayend Completed.*?</td><td>(\d+) (\w+)</td>",
            mail.get_html(),
            _s,
            "Daily OCG Summary: Last Dayend Completed - 数据匹配",
        )
        if _ldc:
            _s.add_report(
                ...
                bool(
                    _ldc
                    and _ldc[0][0] == self.trade_date.last_date_hk.strftime("%Y%m%d")
                    and _ldc[0][1] == "Yes"
                ),
                _ldc[0],
        ...
        _cld = self.findstr(
            r"<tr><td>Coming Logon Date</td><td>(\d+)</td>",
            mail.get_html(),
            _s,
        ...
        if _cld:
            _s.add_report(
                ...
                bool(
                    _cld
                    and _cld[0].strip() == self.trade_date.current.strftime("%Y%m%d")
                ),
                _cld[0],
                self.trade_date.current.strftime("%Y%m%d"),
            )
        ...
        for item in (
            ...
            _gs = self.findstr(
                r"<tr><td>(%s)</td><td>(\w+)</td><td>(\d+)</td><td>(\d+)</td><td>(\d+)</td></tr>"
                % item,
                mail.get_html(),
                _s,
                f"{item} 数据匹配",
    ...
    def check_2_2_0901_daily_pushlog_order_status(self) -> Step:
        ...
        _gs = self.findstr(
            r"<td>(OCG Dealer)</td><td>(\d{8})</td><td>(.{19}).*?</td>",
            mail.get_html(),
            _s,
            "BSS Logon sending at time - 数据匹配",
        )
        if _gs:
            _s.add_report(
                "BSS Logon sending at time is updated to T09:01:00",
                _gs[0][2] == self.trade_date.current.strftime("%Y-%m-%dT09:01:00")
                or _gs[0][2] == self.trade_date.current.strftime("%Y-%m-%dT09:01:01"),
                _gs[0],
                self.trade_date.current.strftime("%Y-%m-%dT09:01:00/01"),
            )
        ...
        for mk, cg, stat in self.findstr(
            ...
            # if cg in ('ETS', 'GEM', 'MAIN', 'NASD'):
            _s.add_report(
                f"HKG and CSC Market Status (Market = {cg})", stat == "OI", stat, "OI"
            )
        ...
        _gs = self.findstr(
            ...
            mail.get_html(),
            _s,
            "Daily HKG Orders Speed Check - 数据匹配完整性",
        )
        if _gs:
            _gs = _gs[0]
            _p_str = ""
    ...
    def check_3_2_kstar_quotes_started(self) -> Step:
        ...
        mail = self.get_mail(
            ...
            f"金仕达V8异常交易系统[{self.trade_date.current.strftime('%Y%m%d')}",
        ...
        for s in ("上海行情", "深圳行情"):
            # _d, _, hq, status
            _gs = self.findstr(
                f"<tr.*?><td.*?>({self.trade_date.current.strftime('%Y%m%d')})</td>"
                f"<td.*?>(行情采集)</td><td.*?>({s})</td><td.*?>(.*?)</td></tr>",
                m_text,
                _s,
                "行情采集 数据匹配",
                1,
            ...
            if _gs:
                _s.add_report(f"「{s}」", _gs[0][3] == "启动成功", _gs[0], "启动成功")
        return _s
    ...
    def check_3_3_exec_job_status(self) -> Step:
        ...
        mail = self.get_mail(
            ...
            f"金仕达V8异常交易系统[{self.trade_date.current.strftime('%Y%m%d')}",
            self.trade_date.current.strftime("%Y-%m-%d 09:14"),
            self.trade_date.current.strftime("%Y-%m-%d 11:17"),
            body="09:15:00",
            step=_s,
        )
        if mail is None:
        ...
        _gs = self.findstr(
            f"<tr.*?><td.*?>({self.trade_date.last_date_cn.strftime('%Y%m%d')})</td>"
    ...
    def check_3_4_trade_data_capture(self) -> Step:
        ...
        for data in ("委托数据", "成交数据"):
            ...
            if _gs:
                dd = _gs[0]
                _s.add_report(
                    dd[1], dd[2] == "成功" or dd[3] == "0", dd, "成功 or 数量=0"
                )
...
if __name__ == "__main__":
    ...
    load_env()
    _td = datetime.date(2026, 9, 2)
    _mc = MorningCheck(_td, cache_db=SCRIPT_DIR / "cache_morning_check.db")
    _mc._main(_mc.check_3_2_kstar_quotes_started)
    _mc.report()
